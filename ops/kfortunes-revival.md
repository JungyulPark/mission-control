...
## 6. 결정 확정 (6/6)
- **언어/시장**: 듀얼 프론트도어(한국어+영문) — 단 AdSense는 **주력 1개 언어로 먼저 뚫고** 확장. 기계번역 다국어 동시 = thin/duplicate 위험.
- **도메인**: **kfortunes.com 유지**(추가비용 0).
- **아키텍처**: **정적 먼저** — 백엔드/firebase/GPT/lemonsqueezy 제거. 결제 영구 제외(사업자 제약). GPT 실시간 → 미리 쓴 풍부한 정적 콘텐츠로 대체(개인화 포기, 읽을거리는 더 풍부).
- **P0 접근**: 바로 새로 설계(frontend-design 스킬) — 단 48edd60의 알찬 자산(115p+결정론 엔진+해설DB)을 토대로 강화·확장(맨땅 아님).
- **운영 모드**: 코드 작업은 각 레포 세션(kfortunes/kstarmatch)에서 분산 진행. **mission-control = 총괄 컨트롤·기억만.**
